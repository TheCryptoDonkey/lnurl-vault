...
src/
  ...
  ui/        display.c (drawing only), buttons.c (thin adapter over
             ...
             library, see Build & flash), the boot note — boot_screen.c
             (the sequence), note_scene.c (rows, no framebuffer),
             guilloche.c (the identity rosette), note_art.c (generated by
             tools/engrave.py, do not edit) — and ui_task.c, the single
             task owning buttons+display for both local note browsing and
...
- **Boot prints a bearer note, because that is what the device holds.** A
  shutter opens onto a blank sheet and the name types itself onto it, big.
  Then the note is printed the way a real one is, in passes, and each pass
  is a boot step: `STORAGE` lays the underprint (the border, sweeping the
  typed name away as it goes), `IDENTITY` the intaglio (a guilloche rosette
  engraved from the device's own key, then the name in an engraved hand),
  `LINK` the numbering (the key's first bytes as a serial). Nothing on the
  note is engraved smaller than the readable floor `font5x7.h` measured; the
  promise line — *PAY THE BEARER ON DEMAND* — and the crest appear only on
  the panel with room for them at that size, the S3. A pass whose subsystem
  failed is simply not printed. Then
  the note turns over and the back is the self-check — the same three
  lines, `OK` or `FAIL`, at the readable size. The rosette is derived from
  the identity key so every vault boots with a different one and yours
  always boots with the same one; a recognition cue, not a proof. The band
  carries the firmware version and board, where a bug report finds them.
  The band and bar wear the idle teal the boot animates into; the note
  itself is engraved in grey, ink on paper, because an engraving is
  monochrome and three attempts in a colour were each too much colour on
  the glass for the type to read over.
  The engraved parts are rendered offline by `tools/engrave.py` into 4-bit
  coverage masks (`src/ui/note_art.c`); the device draws them a row at a
  time with no framebuffer and no font engine (`src/ui/note_scene.c`,
  `src/ui/guilloche.c`). The type is Bodoni Moda, SIL Open Font License
  (`tools/fonts/OFL.txt`). Boot is about ten seconds, a quarter of it the
  self-check held still — read at a desk, not watched as a film; every hold
  is a named constant in `src/ui/boot_screen.c`.
...
Renders every screen the firmware can draw — each printing pass of the boot
note and its back, idle, browse, each confirm verb,
...
The boot animates, and stills cannot judge timing:

```sh
cd test/native && make boot-frames                         # every frame, 1:1
python3 tools/boot_gif.py test/native/preview/frames t-display boot.gif
```

Every `vTaskDelay` the boot takes is captured as a frame held for as long as
it asked, so the GIF runs at the firmware's own timing (Pillow needed for the
GIF; nothing else is). To change the note's engraved parts — the border, the
crest, the name — edit `tools/engrave.py` and rerun it; it regenerates
`src/ui/note_art.c` from the OFL typeface in `tools/fonts/`.
